...
export default class extends Controller {
    ...
    connect() {
        if (isEmpty(this.element.dataset.latitude) && isEmpty(this.element.dataset.longitude)) {
            window.navigator.geolocation.getCurrentPosition((position) => {
                this.setUserCoordinate(position.coords)
    ...
    setDistanceText() {
        this.propertyTargets.forEach((propertyTarget) => {
            ...
            propertyTarget.querySelector('[data-distance-away]').innerHTML = `${Math.round(convertDistance(distanceFrom, 'km'))} kilometres away`
        })
    }
}